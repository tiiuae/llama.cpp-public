...
struct llm_build_falcon_h1 : public llm_graph_context {
    ...
    llm_build_falcon_h1(const llama_model & model, const llm_graph_params & params, ggml_cgraph * gf) : llm_graph_context(params), model(model)  {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            ggml_tensor * ssm_out = build_mamba2_layer(inp->get_recr(), gf, cur, ubatch, il);
    ...
    ggml_tensor * build_mamba2_layer(
        ...
        // {n_embd, d_in_proj} @ {n_embd, n_seq_tokens, n_seqs} => {d_in_proj, n_seq_tokens, n_seqs}
        ggml_tensor * zxBCdt = build_lora_mm(layer.ssm_in, cur);


        // check if the models has ssm_multipliers (MuP)
        if (hparams.ssm_has_mup) {
            struct ggml_tensor * mup_vec = layer.ssm_mup_vec;
        ...
        // conv
        {
            ...
            // and then you're left with the resulting x tensor.
            // For simultaneous sequences, all sequences need to have the same length.
            xBC = ggml_ssm_conv(ctx0, conv_x, layer.ssm_conv1d);
        ...
        // ssm
        {
            ...
            // {n_head, n_seq_tokens, n_seqs}
            dt = ggml_add(ctx0, ggml_cont(ctx0, dt), layer.ssm_dt_b);
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    res = new llama_memory_hybrid(
                        ...
                        /* recurrent_type_k  */ GGML_TYPE_BF16,
                        /* recurrent_type_v  */ GGML_TYPE_BF16,
